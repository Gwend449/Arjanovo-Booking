...
function handle_booking_form() {
    if (isset($_POST['booking_submit'])) {
        ...
        $subject = 'Новая Заявка - "Воронье Гнездо';
        $body = "Заявка получена:\n\n";
        $body .= "Имя: $name\n";
        $body .= "Почта: $email\n";
        $body .= "Телефон: $phone\n";
        $body .= "Въезд-in: $checkin\n";
        $body .= "Выезд-out: $checkout\n";
        $body .= "Кол-во гостей: $guests\n";
        ...
        if (wp_mail($to, $subject, $body, $headers)) {
            ...
            wp_redirect(add_query_arg('booking', 'success', home_url('/#booking')));
            exit;
        } else {
            // Redirect with error message
            wp_redirect(add_query_arg('booking', 'error', home_url('/#booking')));
            exit;
        }
    }
}
